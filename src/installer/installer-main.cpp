...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    try {
        ...
        if (tempPathLen == 0 || tempPathLen >= MAX_PATH) {
            ...
            ShowErrorMessage(
                L"Installation Failed",
                L"Failed to get temporary directory path."
            );
            return 1;
        }
        ...
        if (!ExtractDLLFromResource(tempDLL)) {
            ...
            ShowErrorMessage(
                L"Installation Failed",
                L"Failed to extract plugin files from installer."
            );
            return 1;
        }
        ...
        ShowSuccessMessage(
            ...
            std::wstring(obsPath.begin(), obsPath.end()) + L"\n\n"
        ...
        ShowErrorMessage(
            L"Installation Failed",
            L"An unexpected error occurred during installation."
        );
        exitCode = 1;
    }
